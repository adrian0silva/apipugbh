...
module.exports = async (event) => {
  try {
    ...
    const checkUserExist = await mysql.query('select * from curtidas where email=?', [body.email])
    if (checkUserExist.length) return util.bind(new Error('An account with this email already exists!'))

    const insert = await mysql.query('insert into curtidas (name, email) values (?,?)', [body.name, body.email])
